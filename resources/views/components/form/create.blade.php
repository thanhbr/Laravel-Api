@php
    ...
    $classSplitCol = 'col col-md-'.(12 / $splitColumn);
    $createTitle = $createTitle ?? '';
    $createButton = $createButton ?? __('app.create');
    $storeLink = $storeLink ?? $viewModel->url('store');
    $listLink = $listLink ?? $viewModel->url('index');
    $backLink = $backLink ?? [];

    try{
      if(empty($viewModel)) throw new \Throwable("");
@endphp
  ...
  <style>
    ...
    .app-view.app-view-create .form-actions{max-width:{{$splitColumn * 550}}px;}
    ...
    @hasSection('app-view-tag')
        @yield('app-view-tag')
      @else
        <div class="app-view app-view-create" >
    @endif
    @if ($errors->any())
                    ...
                    <div class="{{$has_error}} as-row form-group row">
                      ...
                      <div class="col control-field d-non">
                        @switch(1)
                          ...
                          @default
                              @if($tag === 'input')
                                <input {!!$attrString!!} value="{{$value}}" />
                              @elseif($tag === 'select' && !empty($option))
                                <select class="form-control select2" {!!$attrString!!}>
                                  @foreach($option as $__k => $__v )
                                    <option value="{{$__v['value']}}" {{$__v['selected']}}>{{$__v['label']}}</option>
                                  @endforeach
                                </select>
                              @else
                                <{{$tag}} {!!$attrString!!} class="form-control" >{{$value}}</{{$tag}}>
                              @endif
                        @endswitch
                      </div>
                      @error($_attr)
              ...
              @else
                <div class="form-actions right row">
                  <button type="button" class="btn" style="background-color: inherit; color: rgb(125, 154, 192);">
                    <a class="" href="{{$listLink}}" style="color:inherit;">
                  ...
                  <button type="submit" tabindex="0" class="btn btn-add mx-auto" style="max-width: 200px">
                    {{$createButton}}
                  </button>
              </div>
              @endif
